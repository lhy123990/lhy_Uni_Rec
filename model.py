...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        self._seq_se_reduce = nn.ModuleDict()
        self._seq_se_expand = nn.ModuleDict()
        ...
        for domain in self.seq_domains:
            ...
            se_hidden = max(1, len(vs) // 2)
            self._seq_se_reduce[domain] = nn.Linear(len(vs), se_hidden)
            self._seq_se_expand[domain] = nn.Linear(se_hidden, len(vs))
    ...
    def _embed_seq_domain(
        ...
        # Feature-wise SE gating: squeeze along emb_dim, excite along feature axis.
        feat_stats = torch.stack([e.mean(dim=-1) for e in emb_list], dim=-1)  # (B, L, S)
        expected_s = se_reduce.in_features
        if feat_stats.shape[-1] != expected_s:
            raise RuntimeError(
                f"SE input feature dim mismatch: got {feat_stats.shape[-1]}, expected {expected_s}. "
                f"This usually means seq tensor side-info count does not match schema-derived vocab config."
            )
        se = se_reduce(feat_stats)
        se = F.relu(se)
        se = torch.sigmoid(se_expand(se)) * 2.0  # (B, L, S)
        emb_list = [e * se[..., i].unsqueeze(-1) for i, e in enumerate(emb_list)]
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        for domain in self.seq_domains:
            tokens = self._embed_seq_domain(
                ...
                self._seq_se_reduce[domain], self._seq_se_expand[domain],
